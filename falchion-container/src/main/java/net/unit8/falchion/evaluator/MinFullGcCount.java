...
public class MinFullGcCount implements Evaluator {
    private static final Logger LOG = LoggerFactory.getLogger(MinFullGcCount.class);
    ...
    public JvmProcess evaluate(Collection<JvmProcess> processes) {
        processes.forEach(p -> {
            ...
            LOG.info("  process id={}, jvmOptions={}, score(FGCT/requests)={}", p.getId(), p.getJvmOptions(), s);
        });
